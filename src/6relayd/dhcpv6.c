...
static void relay_client_request(struct sockaddr_in6 *source, const void *data, size_t len, struct relayd_interface *iface);
static void relay_server_response(uint8_t *data, size_t len);

static int create_socket(uint16_t port);

static void handle_dhcpv6(void *addr, void *data, size_t len, struct relayd_interface *iface);
static void handle_client_request(void *addr, void *data, size_t len, struct relayd_interface *iface);
...
int init_dhcpv6_relay(const struct relayd_config *relayd_config) {
	...
	for(size_t i = 0; i < config->slavecount; ++i) {
		...
		setsockopt(dhcpv6_event.socket, IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP, &mreq, sizeof(mreq));

		if(config->enable_dhcpv6_server) {
			mreq2.ipv6mr_interface = config->slaves[i].ifindex;
			setsockopt(dhcpv6_event.socket, IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP, &mreq2, sizeof(mreq2));
		}
	}
...
static int create_socket(uint16_t port) {
	...
	struct sockaddr_in6 bind_addr = { AF_INET6, htons(port), 0, IN6ADDR_ANY_INIT, 0 };
...
static void handle_nested_message(uint8_t *data, size_t len, uint8_t **opts, uint8_t **end, struct iovec iov[5]) {
	...
	dhcpv6_for_each_option(hdr->options, data + len, otype, olen, odata) {
		if(otype == DHCPV6_OPT_RELAY_MSG) {
			...
			iov[5].iov_len = (((uint8_t *) iov[0].iov_base) + iov[0].iov_len) - (odata + olen);
...
static void handle_client_request(void *addr, void *data, size_t len, struct relayd_interface *iface) {
	...
	struct __attribute__((packed)) {
		...
		.msg_type = DHCPV6_MSG_REPLY, .serverid_type = htons(DHCPV6_OPT_SERVERID), .serverid_length = htons(10), .duid_type = htons(3), .hardware_type = htons(1), .clientid_type = htons(DHCPV6_OPT_CLIENTID), .clientid_buf = { 0 }
	};
	...
	struct __attribute__((packed)) {
		...
		uint16_t len;
		uint16_t value;
	} stat = { htons(DHCPV6_OPT_STATUS), htons(sizeof(stat) - 4), htons(DHCPV6_STATUS_NOADDRSAVAIL) };
	...
	dhcpv6_for_each_option(opts, opts_end, otype, olen, odata) {
		if(otype == DHCPV6_OPT_CLIENTID && olen <= 130) {
			...
			if(olen != ntohs(dest.serverid_length) || memcmp(odata, &dest.duid_type, olen)) {
				return; // Not for us
			}
		}
	}
	...
	if(iov[0].iov_len > 0) { // Update length
		update_nested_message(data, len, iov[1].iov_len + iov[2].iov_len + iov[3].iov_len + iov[4].iov_len - (4 + opts_end - opts));
	}
...
static void relay_server_response(uint8_t *data, size_t len) {
	...
	int32_t ifaceidx = 0;
	struct sockaddr_in6 target = { AF_INET6, htons(DHCPV6_CLIENT_PORT), 0, IN6ADDR_ANY_INIT, 0 };
...
static void relay_client_request(struct sockaddr_in6 *source, const void *data, size_t len, struct relayd_interface *iface) {
	...
	if(h->msg_type == DHCPV6_MSG_RELAY_REPL || h->msg_type == DHCPV6_MSG_RECONFIGURE || h->msg_type == DHCPV6_MSG_REPLY || h->msg_type == DHCPV6_MSG_ADVERTISE) {
	...
	struct sockaddr_in6 dhcpv6_servers = { AF_INET6, htons(DHCPV6_SERVER_PORT), 0, ALL_DHCPV6_SERVERS, 0 };
	struct iovec iov[2] = {
		{&hdr,		sizeof(hdr)},
		       { (void *) data, len	    }
	};
	relayd_forward_packet(dhcpv6_event.socket, &dhcpv6_servers, iov, 2, &config->master);
}
