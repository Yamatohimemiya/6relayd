	...
	if(config->dhcpv6_statefile) {
		...
		for(size_t i = 0; i < config->slavecount; ++i) {
			...
			list_for_each_entry(c, &iface->pd_assignments, head) {
				...
				int l = snprintf(
					leasebuf, sizeof(leasebuf), "# %s %s %x %s %u %x %u ", iface->ifname, duidbuf, ntohl(c->iaid), (c->hostname ? c->hostname : "-"),
...
static void update(struct relayd_interface *iface) {
	...
	for(int i = 0; !change && i < len; ++i) {
		if(addr[i].addr.s6_addr32[0] != iface->pd_addr[i].addr.s6_addr32[0] || addr[i].addr.s6_addr32[1] != iface->pd_addr[i].addr.s6_addr32[1] || (addr[i].preferred > 0) != (iface->pd_addr[i].preferred > 0)
	...
	if(change) { // Addresses / prefixes have changed
		...
		list_for_each_entry_safe(c, d, &iface->pd_assignments, head) {
			...
			if(c->accept_reconf && c->reconf_cnt == 0) {
				...
				struct assignment *a;
				list_for_each_entry(a, &iface->pd_assignments, head) if(a != c && a->clid_len == c->clid_len && !memcmp(a->clid_data, c->clid_data, a->clid_len)) c->reconf_cnt = INT_MAX;
			}
		}
...
static size_t append_reply(uint8_t *buf, size_t buflen, uint16_t status, const struct dhcpv6_ia_hdr *ia, struct assignment *a, struct relayd_interface *iface, bool request) {
			...
			for(size_t i = 0; i < iface->pd_addr_len; ++i) {
				...
				if(a->length < 128) {
					struct dhcpv6_ia_prefix p
						= { .type = htons(DHCPV6_OPT_IA_PREFIX), .len = htons(sizeof(p) - 4), .preferred = htonl(prefix_pref), .valid = htonl(prefix_valid), .prefix = a->length, .addr = iface->pd_addr[i].addr };
					...
					struct dhcpv6_ia_addr n = { .type = htons(DHCPV6_OPT_IA_ADDR), .len = htons(sizeof(n) - 4), .addr = iface->pd_addr[i].addr, .preferred = htonl(prefix_pref), .valid = htonl(prefix_valid) };
			...
			dhcpv6_for_each_option((uint8_t *) &ia[1], end, otype, olen, odata) {
				...
				if(!found) {
					if(otype == DHCPV6_OPT_IA_PREFIX) {
						struct dhcpv6_ia_prefix inv = { .type = htons(DHCPV6_OPT_IA_PREFIX), .len = htons(sizeof(inv) - 4), .preferred = 0, .valid = 0, .prefix = p->prefix, .addr = p->addr };
						...
						struct dhcpv6_ia_addr inv = { .type = htons(DHCPV6_OPT_IA_ADDR), .len = htons(sizeof(inv) - 4), .addr = n->addr, .preferred = 0, .valid = 0 };
...
size_t dhcpv6_handle_ia(uint8_t *buf, size_t buflen, struct relayd_interface *iface, const struct sockaddr_in6 *addr, const void *data, const uint8_t *end) {
	...
	dhcpv6_for_each_option(start, end, otype, olen, odata) {
		...
		list_for_each_entry(c, &iface->pd_assignments, head) {
			if(c->clid_len == clid_len && !memcmp(c->clid_data, clid_data, clid_len) && (c->iaid == ia->iaid || c->valid_until < now) && ((is_pd && c->length <= 64) || (is_na && c->length == 128))) {
				a = c;
		...
		if(hdr->msg_type == DHCPV6_MSG_SOLICIT || hdr->msg_type == DHCPV6_MSG_REQUEST) {
			...
			if(!assigned || iface->pd_addr_len == 0) { // Set error status
				...
				if(hdr->msg_type == DHCPV6_MSG_REQUEST) {
					struct dhcpv6_auth_reconfigure auth = {
						htons(DHCPV6_OPT_AUTH), htons(sizeof(auth) - 4), 3, 1, 0, { htonl(time(NULL)), htonl(++serial) },
							  1, { 0 }
					};
		...
		} else if(hdr->msg_type == DHCPV6_MSG_RENEW || hdr->msg_type == DHCPV6_MSG_RELEASE || hdr->msg_type == DHCPV6_MSG_REBIND || hdr->msg_type == DHCPV6_MSG_DECLINE) {
