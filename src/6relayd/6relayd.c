...
int main(int argc, char *const argv[]) {
	...
	while((c = getopt(argc, argv, "ASR:D:Nsucn::l:a:rt:m:oi:p:dvh:P:")) != -1) {
		switch(c) {
			...
			case 'N': config.enable_ndp_relay = true; break;

			case 's': config.send_router_solicitation = true; break;

			case 'u': config.always_announce_default_router = true; break;

			case 'c': config.deprecate_ula_if_public_avail = true; break;
			...
			case 'r': config.enable_route_learning = true; break;
			...
			case 'm': config.ra_managed_mode = atoi(optarg); break;

			case 'o': config.ra_not_onlink = true; break;
